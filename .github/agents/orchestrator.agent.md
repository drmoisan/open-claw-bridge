...
handoffs:
  ...
  - label: Small-scope implementation path
    ...
    prompt: "Estimate and confirm scope (1-3 production C# files + corresponding tests). If confirmed, execute the short-path development phase against the provided `${feature-folder}` and minimal plan context: baseline capture, implementation, and full QA gates with final CSharpier/.NET analyzer/nullable/MSTest/coverage deltas."
    send: true
...
- strict QA gates,
- final CSharpier/.NET analyzer/nullable/MSTest/coverage deltas,
