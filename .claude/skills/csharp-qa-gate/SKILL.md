---
name: csharp-qa-gate
description: Final QA gate for C# changes. Executes the full CSharpier -> .NET Analyzers -> Nullable Analysis -> MSTest toolchain (with architecture tests), compares against a captured baseline, enforces zero-regression deltas, and produces the required reporting block before the agent declares the change complete.
---
...
Analyzer behavior comes from the .NET SDK defaults and per-project settings this repository actually uses; there is no repo-wide MSBuild props file centralizing analyzer configuration. CSharpier is invoked as the global tool (`csharpier`), not a local `dotnet` tool.

1. `csharpier check .`
2. `dotnet build OpenClaw.MailBridge.sln` (SDK analyzers and nullable reference analysis run as part of the build)
3. `dotnet test OpenClaw.MailBridge.sln --settings mailbridge.runsettings --collect:"XPlat Code Coverage"` (full unit-test pass with coverage; architecture-boundary assertions run as part of the test suite)
...
5. **QA Gate Results** — analyzer, compiler/nullable, MSTest, architecture-test, and coverage deltas. If any step could not be run, mark the corresponding line **unverified** and state why.
