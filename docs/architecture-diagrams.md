...
This feature adds a local-only container path without moving Outlook, COM, or named-pipe access off the Windows host. `OpenClaw.Core` runs as a non-root container with a read-only root filesystem, a writable `/data` volume for SQLite, and a read-only token-file bind mount. The current Windows path remains available as the fallback to `OpenClaw.MailBridge.Client`.
