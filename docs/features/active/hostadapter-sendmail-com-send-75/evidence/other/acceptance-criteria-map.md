...
| AC-11 | Seven-stage toolchain passes; line>=85%, branch>=75%; no regression on changed lines; only documented `[ExcludeFromCodeCoverage]` (integration-covered); no file > 500 lines; additive contracts only | P11-T1..P11-T6 | final-format/lint/typecheck/architecture/test-coverage/coverage-delta artifacts; line 90.25% / branch 79.35%; no file > 500; additive only | PASS |
...
- No file exceeds 500 lines (largest touched: OutlookScanner.cs 465; MailBridgeRuntimeTestDoubles.cs 495;
  PipeRpcWorker.cs 438; Program.cs 436).
