...
- Registers the bridge host as a Windows Service that starts automatically on boot and restarts on failure.
...
- [ ] After install, a Windows Service named `OpenClaw MailBridge` exists, is set to Automatic start, and is running.
- [ ] On host reboot, the service restarts automatically without user intervention.
- [ ] Upgrading the package in-place replaces binaries and preserves existing `bridge.settings.json`.
- [ ] Uninstalling via Settings > Apps removes binaries and the Windows Service (leaves user config in place).
...
- MSIX packaging of Windows Services requires `<uap6:Extension>` service declarations in `Package.appxmanifest` and elevation-capable package trust.
...
- The bridge COM-interop with Outlook requires the user session context; the Windows Service must run as the logged-in user or use a session-aware launch model.
