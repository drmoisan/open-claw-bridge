# Phase 0 Instructions Read — Issue #92
...
Files Read:
- CLAUDE.md — read via harness-loaded standing instructions. No discrete `CLAUDE.md` file exists at the repo root or anywhere in the tree (`**/CLAUDE.md` returns no files); its content is delivered as always-loaded codebase/user instructions in this session context.
- .claude/rules/general-code-change.md — read (design principles, mandatory 7-stage toolchain loop, 500-line file limit, dependency policy: use only approved libraries, minimal change).
...
- .claude/rules/benchmark-baselines.md — read (baseline provenance; not applicable, no benchmark baseline touched by this fix).
...
Notes:
- Scope of this minor-audit fix: bump Microsoft.Data.Sqlite identically in both csproj to clear NU1903 (GHSA-2m69-gcr7-jv3q). No advisory suppression permitted. Sole requirements source: issue.md AC-1..AC-6.
